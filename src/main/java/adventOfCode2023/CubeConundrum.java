...
public class CubeConundrum {
    ...
    private static int sumPossibleGamesIds(String gamesResults) {
        return gamesResults.lines().mapToInt(CubeConundrum::choseGame).sum();
    }
    ...
    private static int choseGame(String gameInfo) {
        ...
        for (int i = 0; i < listOfSubsetsOfCubes.length; i++) {
            ...
            for (int j = 0; j < singleSubset.length; j++) {
                String singleCube = singleSubset[j].strip();
