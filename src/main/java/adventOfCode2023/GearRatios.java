...
public class GearRatios {
    ...
    public static void main(String[] args) {
        ...
        int rows = (int) data.lines().count();
        int columns = data.lines().limit(1).collect(Collectors.joining()).length();
        Character[][] arrayData = new Character[rows][columns];

        for (int i = 0; i < arrayData.length; i++) {
            String line = data.lines().skip(i).limit(1).collect(Collectors.joining());
            for (int j = 0; j < arrayData[i].length; j++) {
                char c = line.charAt(j);
                if (line.charAt(j) == '.') c = 'X';
                arrayData[i][j] = c;
            }
        }
        System.out.println(sumDigits(arrayData));
    }

    private static int sumDigits(Character[][] arrayData) {
        int sum = 0;

        for (int i = 0; i < arrayData.length; i++) {
            String digit = "";
            boolean isDigitToSum = false;

            for (int j = 0; j < arrayData[i].length; j++) {
                char character = arrayData[i][j];

                if (isCharacterNotADigit(character)) {
                    if (isDigitToSum) {
                        sum += Integer.parseInt(digit);
                        isDigitToSum = false;
                    }
                    if (!digit.isEmpty()) digit = "";
                    continue;
                }

                if (isSymbolAdjacent(i + 1, j - 1, arrayData) ||
                        isSymbolAdjacent(i, j - 1, arrayData) ||
                        isSymbolAdjacent(i - 1, j - 1, arrayData) ||
                        isSymbolAdjacent(i - 1, j, arrayData) ||
                        isSymbolAdjacent(i - 1, j + 1, arrayData) ||
                        isSymbolAdjacent(i, j + 1, arrayData) ||
                        isSymbolAdjacent(i + 1, j + 1, arrayData) ||
                        isSymbolAdjacent(i + 1, j, arrayData))
    ...
    private static boolean isCharacterNotADigit(char character) {
        return !Character.isDigit(character);
    }

    private static boolean isSymbolAdjacent(int i, int j, Character[][] arrayData) {
        if (i < 0 || j < 0 || i > (arrayData.length - 1) || j > (arrayData[0].length - 1)) return false;
        return !Character.isLetterOrDigit(arrayData[i][j]);
    }
}
